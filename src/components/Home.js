import React, { useEffect, useState } from "react";
import css from '../styles/style.css';
import css1 from '../styles/style2.css';
import { Link } from "react-router-dom";
import logo from '../images/logo.png'

const Home = () => {
  
  const [data, setData] = useState([])

useEffect(()=> {
  const getalldata = async ()=>{
      //TODO: API call
      const response = await fetch("http://localhost:3000/api/data", {
        
      });
      const json = await response.json();
      console.log(json)
      setData(json)
      
  }

  getalldata();
  const interval = setInterval(  ()=>{
    getalldata();
  },10000)
  return ()=>clearInterval(interval)
},[]);

//Get all data


  return (
    <>
    <div className="root theme-dark" id="root" style={{css ,css1}} >
      <div className="container">
        <div className="align-items-center row justify-content-sm-center">
          <div className="text-center col-12 col-lg-4 col-md-5 col-sm-7">
            <Link to="/">
              <div
                className="padding-none text-center-xs col-12 col-lg-11 col-md-11 col-sm-12"
              >
                <img
                  className="img-logo"
                  src={logo}
                  alt="HodlInfo"
                  width="300"
                  height="68"
                />
              </div>
            </Link>
          </div>

          <div
            className="padding-none text-center col-12 col-lg-4 col-md-2 col-sm-12"
          >
            <div className="btn-group">
              <button
                // onclick="document.getElementById('b').classList.toggle('show')"
                type="button"
                className="header-button btn-secondary dropdown-toggle btn"
              >
                INR
              </button>
              <div tabIndex="-1" role="menu" id="b" className="dropdown-menu">
                <Link
                  value="INR"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/BTC-INR"
                  >INR</Link>
              </div>
            </div>
            <div className="btn-group">
              <button
                type="button"
                aria-haspopup="true"
                aria-expanded="false"
                // onclick="document.getElementById('a').classList.toggle('show')"
                className="dropdown-toggle btn header-button btn-secondary"
              >
                BTC
              </button>
              <div
                tabIndex="-1"
                id="a"
                role="menu"
                aria-hidden="true"
                className="dropdown-menu"
              >
                <Link
                  value="BTC"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/BTC-INR"
                  >BTC</Link>
                <Link
                  value="ETH"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/ETH-INR"
                  >ETH</Link>
                <Link
                  value="USDT"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/USDT-INR"
                  >USDT</Link>
                <Link
                  value="XRP"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/XRP-INR"
                  >XRP</Link>
                <Link
                  value="TRX"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/TRX-INR"
                  >TRX</Link>
                <Link
                  value="DASH"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/DASH-INR"
                  >DASH</Link>
                <Link
                  value="ZEC"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/ZEC-INR"
                  >ZEC</Link>
                <Link
                  value="XEM"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/XEM-INR"
                  >XEM</Link>
                <Link
                  value="IOST"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/IOST-INR"
                  >IOST</Link>
                <Link
                  value="WIN"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/WIN-INR"
                  >WIN</Link>
                <Link
                  value="BTT"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/BTT-INR"
                  >BTT</Link>
                <Link
                  value="WRX"
                  tabIndex="0"
                  role="menuitem"
                  className="dropdown-item"
                  to="/WRX-INR"
                  >WRX</Link>

              </div>
            </div>
            <div className="btn-group">
              <Link
                target="_blank"
                to="https://wazirx.com/invite/sp7pvbt6?utm_source=finstreet&amp;utm_medium=affiliate&amp;utm_campaign=regnow-btn"
                type="button"
                className="header-button btn btn-secondary"
                >BUY BTC
                </Link>
            </div>
          </div>
          <div className="right-header col-12 col-sm-12 col-md-5 col-lg-4">
            <div
              className="d-inline-flex flex-wrap align-items-center justify-content-center"
            >
              <div className="progress-bar-wrapper">
                <svg
                  className="CircularProgressbar"
                  viewBox="0 0 100 100"
                  data-test-id="CircularProgressbar"
                >
                  <path
                    className="CircularProgressbar-trail"
                    d="
                M 50,50
                m 0,-46
                a 46,46 0 1 1 0,92
                a 46,46 0 1 1 0,-92
              "
                    strokeWidth="8"
                    fillOpacity="0"
                    style={{
                      strokeDasharray: '289.027px, 289.027px',
                      strokeDashoffset: '0px'
                    }}
                  ></path>
                  <path
                    className="CircularProgressbar-path"
                    d="
                M 50,50
                m 0,-46
                a 46,46 0 1 1 0,92
                a 46,46 0 1 1 0,-92"
                    strokeWidth="8"
                    fillOpacity="0"
                    style={{
                      strokeDasharray: '289.027px, 289.027px',
                      strokeDashoffset: '62.6224px'
                  }}
                  ></path>
                  <text className="CircularProgressbar-text" x="50" y="50" id="count"></text>
                </svg>
              </div>
              <Link className="color-white" to="/"
                ><div
                  className="d-flex telegram-logo-text header-telegram-button btn align-items-center pointer color-white"
                >
                  <div className="d-inline-block">
                    <img
                      src="data:image/png;base64,"
                      className="telegram-logo"
                      alt="#"
                    />
                  </div>
                  <div className="text-nowrap d-inline-block color-white">
                    Connect Telegram
                  </div>
                </div></Link>
              <div className="margin-10 d-inline-block">
                <label className="switch"
                  ><input
                    // onclick="themeChange()"
                    /><span className="slider round"></span
                ></label>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="container2">
        <div className="Container2-1">
          <div className="flexbox" style={{ padding: "10px 0px"}}>
            <div className="text-center">
              <div className="average-header-maintext color-green">0.59 %</div>
              <div className="average-header-subHeading">5 Mins</div>
            </div>
            <div className="text-center">
              <div className="average-header-maintext color-green">0.62 %</div>
              <div className="average-header-subHeading">1 Hour</div>
            </div>
            <div style={{maxWidth: '40%'}}>
              <div className="text-center font-32 average-block">
                <div className="average-subText">
                  <span className="subText-heading">Best Price to Trade</span>
                </div>
                <div className="average-heading" style={{paddingBottom: "10px"}}>
                  ₹ 24,47,285
                </div>
                <div className="average-subText">
                  Average BTC/INR net price including commission
                </div>
              </div>
            </div>
            <div className="text-center">
              <div className="average-header-maintext color-green">8.25 %</div>
              <div className="average-header-subHeading">1 Day</div>
            </div>
            <div className="text-center">
              <div className="average-header-maintext color-green">16.71 %</div>
              <div className="average-header-subHeading">7 Days</div>
            </div>
          </div>
        </div>

          <table   style={{padding:'400px'}}>
            <thead>
              <tr>

                <th>
                  <h4>Name</h4>
                </th>
                <th>
                  <h4>Last Traded Price</h4>
                </th>
                <th>
                  <h4>Buy / Sell Price</h4>
                </th>
                <th>
                  <h4>Volume</h4>
                </th>
                <th>
                  <h4>Base Unit</h4>
                </th>
              </tr>
            </thead>

            {data.map(i=>{
              return(
                <tbody style={{marginTop:'20px'}}>
                <tr>
                 
                  <td>{i.name}</td>
                  <td>{i.last}</td>
                  <td>{i.buy} / {i.sell}</td>
                  <td>{i.volume}</td>
                  <td>{i.base_unit}</td>
                </tr>
                  </tbody>
              )
            })}
          </table>

      </div>
      <div className="footer" style={{padding: "0 20px"}}>
        <div className="d-flex align-items-center text-center">
          <div className="footer-text">Copyright © 2019</div>
          <div className="footer-text">HodlInfo.com</div>
          <div className="footer-text pointer" style={{marginLeft: "auto"}}>
            <Link to="mailto:support@hodlinfo.com" className="footer-text-link"
              >Support</Link>

          </div>
        </div>
      </div>
    </div>
    <footer>
      <div id="footer">
        <div>
          <Link>add hodlinfo to home screen</Link>
        </div>
      </div>
    </footer>
    </>
  );
};

export default Home;
